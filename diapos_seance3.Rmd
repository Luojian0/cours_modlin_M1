...
La régression linéaire se caractérise par des variables explicatives *quantitatives* (Ex : taille). 
L’analyse de la variance (ANOVA) se caractérise par des variables explicatives *qualitatives* (Ex : sexe).

Dans les deux cas la variable expliquée est *quantitatives* (ex : poids).
...
Cette base de données comprend le comptage du nombre d‘abeille (count, numeric) par parcelle agricole en fonction de différents type de spray utilisé sur les parcelles (spray, factor).
...
On cherche à expliqué nombre d’abeilles (Y) en fonction du type de spray utilisé (X).


```{r}
layout(matrix(1:3, 1, byrow=TRUE), respect=TRUE)
p = plot(d$spray, d$count, main="count~spray", las=2)
m = lm(count ~ spray, d)
# spray = jitter(as.numeric(d$spray))
# arrows(spray, d$count, spray, d$count-m$residuals, col=adjustcolor(4, alpha.f=0.5), length=0.1)
...
La moyenne observée du groupe $i$ avec $i = 1,...,I$ est
$$\overline{Y}_i = \frac{1}{J}\sum^J_{j=1}Y_{ij}$$

La variance observée du groupe $i$ avec $i = 1,...,I$ est
...
Les résidus {e_{ij} } sont associés, sans en être des réalisations, aux variables erreurs {"ij } qui sont inobservables et satisfont aux 3 conditions suivantes :
